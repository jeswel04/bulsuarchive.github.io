...
class Htry extends Controller{
    ...
    function index(){
        ...
        $data['history'] = $history->fetchall('id');
        if(!Auth::logged_in()){
            redirect('login');
        }
            if(!empty($history)){
                $data['research']=$research->hitry();
            }
                if(isset($_POST['dept'])){
            ...
            if(isset($_POST['delete'])){
                $history->delete($data['history'][0]['id']);
                redirect('htry');
            }
        ...
        $this->view('auth/admin/history',$data);
    }
 }