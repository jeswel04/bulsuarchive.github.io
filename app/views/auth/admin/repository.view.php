...
<div class="container">
  ...
  <h1 style="margin-top: 1vh;">Repository</h1>
  ...
  <?php
      if(!empty(isset($_POST['search']))){
        if(!empty($search)){
            foreach($search as $keys=>$value){
              ...
              "<a href=\"".ROOT."/file?id=".$search[$keys]['token']."\">".
              "<h2 style=\"color:var(--color-primary);\">".$search[$keys]['title']."</h2>".
              "<span>".$search[$keys]['researchers']."</span>".
              "<br><span>".$search[$keys]['adviser']."</span>".
              "<p>".$search[$keys]['department_code']." - ".$search[$keys]['department_description']."</p>".
              "<p>".$search[$keys]['course_code']." - ".$search[$keys]['course_description']."</p>".
              "<p>".$search[$keys]['description']."</p>".
    ...
    <?php
        foreach($research as $key=>$title){
          echo
              "<div class=\"cardUpload\" style=\"min-height:20%\">".
              "<form method=\"POST\">".
              "<button class=\"btn_download\" type=\"submit\" id=\"saved\" name=\"saved\" style=\"background-color: transparent;\"><span class=\"material-symbols-sharp\" style=\"color:var(--color-info-dark);\">bookmark".
              "</span></button></form>".
              "<a href=\"".ROOT."/file?id=".$research[$key]['token']."\">".
              ...
              "<span>".$research[$key]['date_publish']."</span>".
              "<h3> VIEWS: ".$research[$key]['views']." | DOWNLOADS: ".$research[$key]['downloads']."</h3>".
              "</a></div>";
          }
          if(isset($_POST['saved'])){
            ...
            if($saved->where(['token'=>$research[$key]['token']])){
                $saved->update($research[$key]['id'],$_POST);
            }else{
            $saved->insert($_POST);
            }
          }
