...
<div class="container">
    ...
    <div class="cardContainer" style="margin-top: 10px;">
        <div class="child">
        <h3><?=$tempuser[0]['picture']?>-<?=$tempuser[0]['lastname']?></h3>
        <iframe id="iframe" name="iframe" src="../upload_files/<?=$tempuser[0]['picture']?>#toolbar=0" frameborder="0"></iframe>
        </div>
        <?foreach ($tempuser as $tempuser):?>
        <form method="POST">
            <input type = "hidden" name="csrf_token" id="csrf_token" value=<?=User::createToken()?>></input>
            <h3>School Id</h3>
            <input class="field" type="number" id="school_id" name="school_id" value="<?=$tempuser[0]['school_id']?>"></input>
            <div class="grid3">
                <div class="">
                    <h3>User Type</h3>
                    <select name="role_id" id="role_id" type="text">
                        <option value="<?=$tempuser[0]['role_id']?>" selected="selected"><?=$tempuser[0]['role_id']?></option>
                ...
                <div class="">
                    <h3>Department</h3>
                    <select type="text" id="department_id" name="department_id" type="text">
                        <option value="<?=$tempuser[0]['department_id']?>"><?=$tempuser[0]['department_id']?></option>
                    </select>
                </div>
                <div class="">
                    <h3>Date Sign Up</h3>
                    <input class="field" type="date" value="<?=$tempuser[0]['date_validate']?>">
                </div>
            </div>
            <h3>Course</h3>
            <select id="course_id" name="course_id" type="text">
                <option value="<?=$tempuser[0]['course_id']?>"><?=$tempuser[0]['course_id']?></option>
                <option value="3">Faculty</option>
                <option value="2">Student</option>
            </select>
            <h3>Last name</h3>
            <input class="field" type="text" id="lastname" name="lastname" value="<?=$tempuser[0]['lastname']?>">
            <h3>First name</h3>
            <input class="field" type="text" id="firstname" name="firstname" value="<?=$tempuser[0]['firstname']?>">
            <h3>BULSU email</h3>
            <input class="field" type="text" id="email_verified" name="email_verified" value="<?=$tempuser[0]['email']?>">
            <button type="submit" id="accept_account" class="field_btn" name="accept_account">Accept</button>
            <button type="submit" id="decline" class="field_btn_list" name="decline">Decline</button>
        </form>
    <?endforeach;?>
    </div>
  </main>
